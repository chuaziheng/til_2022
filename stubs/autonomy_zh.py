...
def get_all_points(map_) -> List[GridLocation]:
    all_points = set()
    ...
    for w in range(width):
        for h in range(height):
            ...
            if map_.passable(grid_loc) and map_.in_bounds(grid_loc):
                all_points.add(grid_loc)
    return all_points

def get_dfs_path(cur_pose, not_visited_set, map_):
    not_visited = not_visited_set.copy()
    stack = []
    exploration_path = []

    cur_pose = map_.real_to_grid(cur_pose)
    stack.append(cur_pose)

    while not_visited:
        while stack:
            cur_wp = stack.pop()
            neighbours = map_.neighbours(cur_wp)
            for n_wp, n_cost, grid_value in neighbours:
                if n_wp in not_visited:
                    exploration_path.append(n_wp)
                    not_visited.remove(n_wp)
                    stack.append(n_wp)

        if not stack and not_visited:
            temp = list(not_visited.copy())
            temp.sort(key=lambda l: euclidean_distance(l, cur_wp), reverse=True)
            jump_to = temp.pop()
            stack.append(jump_to)

    return [map_.grid_to_real(i) for i in exploration_path]
    ...
    while True:
        ...
        if not curr_loi:
            ...
            if path:
                ...
                if not curr_wp:
                    curr_wp = path.pop()
                    set_prev_len = len(not_visited_set)
                    not_visited_set.remove(map_.real_to_grid(curr_wp))
                    print('updating set ', set_prev_len, len(not_visited_set))
